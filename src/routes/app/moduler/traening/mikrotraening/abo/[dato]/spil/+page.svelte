<script lang="ts">
	...
	// Konstant program-id til pause-mekanik (ingen reel collection — bruges
	...
	const programId = $derived<string>(ABO_PROGRAM_ID);
	...
	onMount(async () => {
		...
		try {
			...
			try {
				const gemt = await hentPause(u.uid, programId, dagNummer);
	...
	function gemAktivPause() {
		...
		void gemPause(u.uid, {
			productId: produktType === 'premium' ? 'premiumabo' : 'basisabo',
	...
	async function stopOgForlad() {
		stopTimer();
		const u = user;
		if (u && phase !== 'done' && !traeningGennemfort) {
			try {
