<script lang="ts">
	...
	// (samme logik som forsiden og program-detail-pagen). 1-indekseret.
	...
	$effect(() => {
		...
		void logTraening(u.uid, {
			...
			kilde: 'tildelt',
			programId,
			programNavn: programData?.program.navn ?? 'Tildelt program',
			forlobId,
	...
	function stopOgForlad() {
		stopTimer();
		goto(`/app/moduler/traening/program/${forlobId}/${programId}`);
	}
</script>
...
<div class="player">
	...
	{:else if phase === 'done'}
		<div class="finish-card">
			...
			<p class="finish-sub">Godt arbejde med {programData?.program.navn}.</p>
