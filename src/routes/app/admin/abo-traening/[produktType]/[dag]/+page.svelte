<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			alleOvelser = ovelser;
			if (!data) {
				fejl = 'Programmet er ikke oprettet endnu. Auto-generér det først.';
				return;
			}
	...
	function tilfoejOvelse() {
		...
		const config = programData?.program.genererConfig;
		exercises = [
			...exercises,
			{
				exerciseId: valgtNyOvelse,
				sets: config?.sets ?? 3,
				workSec: config?.workSec ?? 30,
				restSec: config?.restSec ?? 10,
				bonus: false
			}
		];
		valgtNyOvelse = '';
	...
	async function gem() {
		...
		try {
			...
			await gemAboMikrotraeningDag(produktType, dag);
			gemStatus = 'gemt';
			gemBesked = 'Gemt.';
		} catch (e) {
			gemStatus = 'fejl';
...
<div class="page">
	<header class="page-header">
		<a class="back" href="/app/admin/abo-traening">
