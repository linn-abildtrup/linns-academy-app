<script lang="ts">
	...
	async function gemMetadata() {
		...
		try {
			await gemMasterProgram(programId, {
				...programData.program,
				navn,
	...
	async function genererStandardprogram() {
		...
		try {
			...
			await gemMasterDage(programId, nyeDage);
...
<div class="page">
	<header class="page-header">
		<a class="back" href="/app/admin/programmer">
	...
	{:else if programData}
		<section class="form-card">
			...
			<label class="checkbox-rad">
				...
				<span>Aktiv (kan tildeles og vælges)</span>
			</label>
			{#if metaFejl}
		...
		<section class="actions-card">
			...
			<div class="config-grid">
				...
				<label class="config-felt">
					...
					<input
						...
						oninput={(e) => setConfig('workSec', parseInt((e.target as HTMLInputElement).value, 10))}
					/>
				</label>
				<label class="config-felt">
					...
					<input
						...
						oninput={(e) => setConfig('restSec', parseInt((e.target as HTMLInputElement).value, 10))}
					/>
				</label>
			</div>
			...
			<p class="hint">
				...
				hver med {genConfig.sets} sæt × {genConfig.workSec}s arbejde × {genConfig.restSec}s hvile. Du
				kan justere hver dag bagefter.
			</p>
			...
			<div class="dage-liste">
				{#each programData.dage as dag (dag.dagNummer)}
					<a
						...
						href="/app/admin/programmer/{programId}/{dag.dagNummer}"
					>
