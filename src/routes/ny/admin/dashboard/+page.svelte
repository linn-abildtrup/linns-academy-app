<script lang="ts">
	// ============================================================
	// Dashboard, i det nye design.
	//
	// Nittende og SIDSTE af de gamle admin-sider, 1. september 2026, og
	// den stoerste med godt 2.000 linjer.
	//
	// HELE SIDEN ER KOPIERET ORDRET, baade script, markup og stil. Der er
	// ikke tegnet noget forfra, og det er en bevidst beslutning: siden
	// rummer 897 linjer tal, tabeller og ti grafer om kundernes udvikling.
	// At skrive markup forfra ville betyde at hver eneste tabel skulle
	// maerkes op igen, og en forkert overskrift paa et tal her er vaerre
	// end at siden ser gammel ud. Det er tallene paa forretningen.
	//
	// UDSEENDET SKIFTER VIA EN FARVEBRO nederst i stilen: de gamle
	// farve-navne peger paa de nye vaerdier. Saa foelger hele siden det nye
	// design uden at en eneste linje markup er roert.
	//
	// Det eneste der er aendret er admin-tjekket og at toppen er fjernet,
	// fordi rammen allerede har en.
	//
	// Den gamle side paa /app/admin/dashboard er uroert.
	// ============================================================
	...
	function bygRejseGraf(punkter: RejsePunkt2[], fastYMax: number | null) {
		...
		const gx = (i: number) => G.padL + (n <= 1 ? 0 : (i / (n - 1)) * (G.w - G.padL - G.padR));
...
{#if !maaVaereHer}
	<p class="da-kun">Siden er kun for admin.</p>
{:else}
	<div class="page">
		<header class="page-header">
			...
			<p class="page-sub">
				...
				{:else if aktivMaaling === 'velvaere'}
					Vores egen velvære-måling (energi, mave, cravings, humør, søvn) — højere score er bedre.
					En anden måling end MRS.
		...
		{:else if snapshot && scope}
				...
				{:else}
					...
					<p class="figur-note">
						En refleksion = en forløbsdag hvor kunden skrev et svar. Vist for {reflValg === 'samlet'
							...
							: reflValg === 'kickstart'
								...
								: reflValg === 'kropsro'
									? 'alle Kropsro-hold'
									: (reflSnapshot.prForlob.find((f) => f.forlobId === reflValg)?.navn ??
										'forløbet')}.
					</p>
					{#if rs.prDag.length > 0}
						...
						<section class="card">
							...
							<p class="skala-note">
								Højeste søjle = {maxAntal} kunder. Hver søjle = én forløbsdag.
							</p>
						</section>
					{/if}
					<section class="card">
						...
						{#if reflValg === 'samlet'}
							<p class="figur-note">
								Vælg et <strong>forløb</strong> eller <strong>alle Kickstart/Kropsro</strong> ovenfor
								for at opsummere en bestemt dags refleksioner.
							</p>
						{:else}
							<p class="figur-note">
								...
								<div class="akt-rad">
									...
									<div class="akt-meta">
										...
										{#if k.status === 'inaktiv'}
											...
											>
										{:else}
											<span class="akt-status faldende"
												>{k.sidste7} vs {k.forrige7} (uge nu/før)</span
											>
										{/if}
									</div>
								</div>
					...
					{#if aktivMaaling === 'mrs'}
						...
						<p class="figur-note">
							...
							{mrsPop === 'gennemfoerte'
								...
								: 'kunder med mindst 2 MRS-skemaer'} ({scopeNavn}). Gns. forbedring = hvor mange
							point scoren i snit er faldet; er blevet bedre = andelen der er gået ned.
						</p>
						{#if harNokMrs}
							<!-- Rejse-graf -->
							{#if graf}
								<section class="card">
									...
									<p class="figur-note">
										...
										>.
										{mrsPop === 'gennemfoerte' ? 'Kun gennemførte kunder. ' : ''}{aktivFane ===
										'alle'
							...
							<section class="card">
								...
								<div class="fordeling-tegnforklaring">
									...
									>
									<span
										><span class="fb-prik vaerre"></span>Værre ({mrsData.forbedringsFordeling
											.vaerre} ·
			...
			{#snippet rejseKort(
				titel: string,
				note: string,
				graf: ReturnType<typeof bygRejseGraf>,
				farve: string,
				tomTekst: string
			)}
					...
					{#if graf}
						<svg class="graf" viewBox="0 0 {G.w} {G.h}" preserveAspectRatio="xMidYMid meet">
							...
							{#if graf.kickMidX !== null}
								<text x={graf.kickMidX} y={10} class="fase-label" text-anchor="middle"
									>Kickstart</text
								>
							{/if}
			...
			<div class="opdateret">
				...
				<span class="opdateret-note">
					...
					<strong>Fuld genberegning</strong> læser alle kunder — brug den efter profil-rettelser eller
					slettede målinger.
				</span>
			</div>
		{/if}
...
<style>
	...
	/* ============================================================
	   FARVEBRO. Hele siden er kopieret ORDRET fra den gamle admin,
	   markup og stil, fordi den rummer 897 linjer tal, tabeller og ti
	   grafer. At tegne den forfra ville betyde at hver eneste tabel
	   skulle maerkes op igen, og en forkert overskrift paa et tal her
	   er vaerre end at siden ser gammel ud: det er tallene paa
	   forretningen.
	   I stedet peger de gamle farve-navne paa de nye vaerdier. Saa
	   skifter hele siden udseende uden at en eneste linje markup er
	   roert. Se samtalen 1. september.
	   ============================================================ */
	.page {
		--bg: #fbf8f2;
		--white: #f6f0e7;
