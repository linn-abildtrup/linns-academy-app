...
const EMAIL = 'aovergaard@live.dk';
...
(async () => {
	...
	if (a.exists) {
		...
		console.log(`  accessSource: ${d.accessSource}`);
		console.log(`  status: ${d.status}`);
	} else {
		console.log('Ingen allowedEmail-doc for denne email.');
		// Søg via email-felt
		const q = await db.collection('allowedEmails').where('email', '==', EMAIL).get();
		if (!q.empty) {
	...
	if (u.empty) console.log('=== userDoc: ikke fundet (har ikke logget ind eller registreret) ===');
